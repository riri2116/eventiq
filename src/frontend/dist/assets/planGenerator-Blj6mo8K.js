import { r as reactExports, j as jsxRuntimeExports, R as React, b as reactDomExports, v as vt } from "./index-jC9o-HqV.js";
import { a as composeRefs, u as useComposedRefs } from "./button-D9ymTIjJ.js";
import { v as vendorDatabase16, b as VENDOR_EMOJI_16 } from "./vendorDatabase-C2PClaxl.js";
...
function pickVendor16(key, tier) {
  const entry = vendorDatabase16[key];
  if (!entry) return void 0;
  const vendors = entry.dehradun;
  if (!vendors || vendors.length === 0) return void 0;
...
function buildVendors16(keys, tierPref) {
  const result = {};
...
function buildBestFitVendors16(keys, budget) {
  const result = {};
  let remaining = budget;
  const initialTier = budget >= 5e6 ? "$$$" : budget >= 5e5 ? "$$" : "$";
  for (const key of keys) {
    const entry = vendorDatabase16[key];
    ...
    const chosen = sorted.find((v) => v.cost <= perVendorShare) ?? vendors.find((v) => v.tier === initialTier) ?? vendors[vendors.length - 1];
    const vendorFull = {
      ...chosen,
      categoryKey: key,
      emoji: VENDOR_EMOJI_16[key] ?? "📦"
    };
    result[key] = vendorFull;
    remaining -= chosen.cost;
  }
  return result;
}
function calcTotal16(vendors) {
  return Object.values(vendors).reduce((sum, v) => sum + ((v == null ? void 0 : v.cost) ?? 0), 0);
}
function makeId() {
  return `plan_${Date.now()}_${Math.random().toString(36).slice(2, 7)}`;
}
function generatePlans(form) {
  const {
    eventName,
    eventType,
    category,
...
function savePlanToStorage(planSet, userEmail) {
...
function loadPlansFromStorage(userEmail) {
  const key = `eventiq_plans_${userEmail}`;
...
export {
  ...
  Close as a,
  createDialogScope as b,
  composeEventHandlers as c,
  createSlottable as d,
  createContextScope as e,
  Trigger as f,
  generatePlans as g,
  loadPlansFromStorage as l,
  savePlanToStorage as s
};
